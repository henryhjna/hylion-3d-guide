...
[CAN 버스] ──┬─────┬─────┬─────  ... 데이지체인 (다리당 6개)
   ...
   │  │  L6387 게이트드라이버 ×3         │  │
   │  │      │                        │  │
   │  │  STL180N6F7 × 6 ─ 3상 출력 ───┼──┼──> BLDC 모터
   │  │      ▲                        │  │       (U/V/W)
   │  │  3-shunt 전류센싱              │  │
   │  │  (STM32 내장 OP-amp 활용)      │  │
   │  │                               │  │
   │  │  FDCAN + 온보드 CAN 트랜시버    │  │
   │  │  (120Ω 종단 내장, 펌웨어 제어) ─┼──┼──> CAN 버스
   │  └───────────────────────────────┘  │
   │           │ I2C (PB8=SCL, PB7=SDA 권장)│
   │  ┌────────┴────────┐                │
   │  │ AS5600 자기엔코더 │<── 모터축 자석  │
   │  │  (0x36, 12-bit)  │    (직경 방향   │
   │  └──────────────────┘    자화)       │
   │                                     │
   │  펌웨어: Recoil-Motor-Controller-BESC │
   │  (T-K-233, C, MIT license)           │
   └─────────────────────────────────────┘
```
...
| 구성요소 | 상세 | 내장/별도 |
|---|---|---|
| **B-G431B-ESC1 보드** | STM32G431CB (170 MHz Cortex-M4, 내장 OP-amp) + L6387 게이트드라이버 ×3 + STL180N6F7 ×6 + 3-shunt 전류센싱 + FDCAN·온보드 CAN 트랜시버(120Ω 종단 내장) | 보드 하나로 통합 |
| **AS5600 자기 엔코더** | 12-bit, I2C 주소 0x36, PB8=SCL / PB7=SDA 권장. R1/R4 제거 + DIR→VDD 수정 필수 | 별도 부품 |
| **자석 (diametric)** | AS5600 동봉, **직경 방향 자화** (원기둥 지름 가로로 N/S) | AS5600에 포함 |
| **BLDC 모터** | MAD M6C12 150KV (hip·knee) or MAD 5010 (ankle) | 별도 부품 |
| **사이클로이드 기어박스** | 3D 프린트, 15:1 감속 | 3D 프린트 |
| **Recoil 펌웨어** | T-K-233/Recoil-Motor-Controller-BESC (C, MIT) — STM32CubeIDE로 플래시 (Run ×4 절차) | 오픈소스 |
