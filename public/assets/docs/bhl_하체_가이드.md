...
## 2. BESC(모터 컨트롤러) 1세트 구성

BHL이 "모터 컨트롤러"를 부르는 명칭이 **BESC** (Brushless ESC) 입니다. 조립 관점에서는 1 관절 = 1 BESC.

- **하드웨어**: B-G431B-ESC1 보드 (ST Discovery Kit)
  - MCU: STM32G431CB
  - 게이트 드라이버: L6387
  - 파워 MOSFET: STL180N6F7
  - ST-LINK 도터보드 (플래시용)
- **센서**: AS5600 자기 엔코더 + **반경 방향(radial) 자화 자석** ⚠️
- **펌웨어**: T-K-233/Recoil-Motor-Controller-BESC (C, MIT 라이선스, STM32CubeIDE로 플래시)
- **모터**:
  - M6C12 150KV BLDC 드론 모터 (6512 액추에이터용)
  - 5010 BLDC 드론 모터 (5010 액추에이터용, variant: 110KV / 310KV / 370KV — 펌웨어 프로파일로 선택)
- **운용 전압**: 24V (BHL 논문 성능 평가 기준)

> **용어 관계**: BESC = B-G431B-ESC1 하드웨어 + Recoil 펌웨어. "ESC"와 "BESC"는 사실상 같은 것을 가리킴.
