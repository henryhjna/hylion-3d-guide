...
export const PARTS = {
  ...
  left_arm: {
    ...
    specs: [
      ...
      { label: '프로토콜', value: 'Feetech SCServo (SMS/STS), 1 Mbps — Dynamixel Protocol 2.0과 다름' },
      { label: '제어 주기', value: 'LeRobot 표준 ~30 Hz (카메라 동기) / 순수 서보 sync는 100 Hz+까지 가능' },
      { label: '기어비', value: 'Follower 전 관절 1/345 (LeRobot 공식). Leader는 관절별 1/191·1/345·1/147 혼합' },
      { label: '서보 variant', value: 'STS3215 Pro 12V (stall 30 kg·cm) — 기획서 §7.6 확정 선택' },
      { label: '그리퍼 jaw', value: '~5~6 cm (3D 프린트, Blender 스펙시트로 W-1 확정)' },
  ...
  left_leg: {
    ...
    specs: [
      { label: 'DOF', value: '6DOF (다리당) — hip roll/yaw/pitch + knee pitch + ankle pitch/roll' },
      { label: '액추에이터', value: '3D프린트 사이클로이드 기어박스 (감속비 15:1, BHL 공식) + BLDC + AS5600' },
      { label: '모터', value: 'MAD M6C12 150KV ×4 (hip·knee) + MAD 5010 ×2 (ankle) — 다리당, 극쌍 14, delta 와인딩' },
      { label: '드라이버 (BESC)', value: 'B-G431B-ESC1 ×6 — 다리당 6 보드 (보드당: STM32G431CB + L6387 ×3 + STL180N6F7 ×6 + 3-shunt + FDCAN 트랜시버). Recoil-Motor-Controller-BESC 펌웨어, FOC 수 kHz' },
      { label: '통신', value: 'CAN 2.0 버스 ×1, 1Mbps (BHL 원본 CAN ID 체계 유지: 좌 1/3/5/7/11/13, 우 2/4/6/8/12/14)' },
      { label: '제어', value: 'NUC + RL Policy MLP via ONNX, policy 25Hz (CAN 제어 루프 250Hz — BHL 논문 기준)' },
      { label: '특성', value: '백드라이버블 — 전원 차단 시 안전 주저앉음' },
    ],
    walkingRL: {
    ...
    safety: 'IM10A IMU (USB 직결) → NUC 낙상 감지 → 모터 토크 해제 (백드라이버블 → 안전 주저앉음)',
